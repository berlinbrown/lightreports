...
import com.octane.util.junit.framework.AssertionFailedError;
import com.octane.util.junit.framework.Test;
import com.octane.util.junit.framework.TestListener;
...
class TestSuitePanel extends JPanel implements TestListener {
    ...
    public Component getTreeCellRendererComponent(
      ...
      if (model instanceof TestTreeModel) {
        ...
        if (testModel.isFailure(t)) {
          if (fFailureIcon != null) setIcon(fFailureIcon);
          s = " - Failed";
        } else if (testModel.isError(t)) {
          if (fErrorIcon != null) setIcon(fErrorIcon);
          s = " - Error";
        } else if (testModel.wasRun(t)) {
          if (fOkIcon != null) setIcon(fOkIcon);
          s = " - Passed";
        }
        if (c instanceof JComponent) ((JComponent) c).setToolTipText(getText() + s);
      }
  ...
  public Test getSelectedTest() {
    ...
    if (paths != null && paths.length == 1) return (Test) paths[0].getLastPathComponent();
    return null;
  }
