...
public class LoadingTestCollector extends ClassPathTestCollector {
  ...
  Class classFromFile(String classFileName) throws ClassNotFoundException {
    ...
    if (!fLoader.isExcluded(className)) return fLoader.loadClass(className, false);
    return null;
  }

  boolean isTestClass(Class testClass) {
    if (hasSuiteMethod(testClass)) return true;
    if (Test.class.isAssignableFrom(testClass)
        && Modifier.isPublic(testClass.getModifiers())
        && hasPublicConstructor(testClass)) return true;
    return false;
  }
